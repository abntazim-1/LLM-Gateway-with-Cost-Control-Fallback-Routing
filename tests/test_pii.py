...
def test_pii_sanitizer_text():
    ...
    assert (
        sanitizer.sanitize_text(text_with_email)
        == "Please contact me at [EMAIL_REDACTED]."
    )
    ...
    assert (
        sanitizer.sanitize_text(text_with_cc)
        == "My card number is [CREDIT_CARD_REDACTED]."
    )
...
def test_pii_sanitizer_messages():
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "Email is test@example.com and phone is 555-555-5555.",
        },
    ]
