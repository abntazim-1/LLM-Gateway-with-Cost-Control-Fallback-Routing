...
async def test_router_complexity_routing_simple():
    ...
    ranked_why = await router.get_ranked_adapters(messages=[{"role": "user", "content": "Why is the sky blue?"}])
    assert ranked_why[0].id == "cheap-model"

    ranked_short = await router.get_ranked_adapters(messages=[{"role": "user", "content": "Why?"}])
...
async def test_router_complexity_routing_complex():
    ...
    ranked = await router.get_ranked_adapters(
        messages=[{"role": "user", "content": "Please write a high performance code to optimize and benchmark this algorithm proof."}]
    )
    assert len(ranked) == 2
    ...
    ranked_code = await router.get_ranked_adapters(messages=[{"role": "user", "content": code_prompt}])
    assert ranked_code[0].id == "premium-model"

    # Large token volume routes to premium
    large_prompt = "Analyze this text in detail: " + ("lorem ipsum dolor sit amet " * 150)
    ranked_large = await router.get_ranked_adapters(messages=[{"role": "user", "content": large_prompt}])
...
async def test_router_capability_aware_routing():
    ...
    router = Router(adapters=[standard, json_capable], circuit_registry=registry, strategy="cost_first")
    
    # Normal request selects standard (cheapest)
    ranked_normal = await router.get_ranked_adapters(messages=[{"role": "user", "content": "hello"}])
...
async def test_router_required_capabilities_filtering():
    ...
    basic = MockRoutingAdapter({
        "id": "basic-model", "model": "m1",
        "cost_per_1k_prompt": 0.001, "cost_per_1k_completion": 0.002,
        "capabilities": ["json_mode"],
    })
    advanced = MockRoutingAdapter({
        "id": "advanced-model", "model": "m2",
        "cost_per_1k_prompt": 0.01, "cost_per_1k_completion": 0.02,
        "capabilities": ["json_mode", "vision", "function_calling"],
    })

    router = Router(adapters=[basic, advanced], circuit_registry=registry, strategy="cost_first")
