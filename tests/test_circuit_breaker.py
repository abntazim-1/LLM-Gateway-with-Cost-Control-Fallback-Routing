...
async def test_circuit_breaker_recovery_via_health_check():
    ...
    ledger = LedgerStore(":memory:")
    cb = CircuitBreaker(backend_id="hc_backend", ledger=ledger, failure_threshold=2, cooldown_sec=9999)
...
async def test_circuit_breaker_half_open_failure_reopens():
    ...
    ledger = LedgerStore(":memory:")
    cb = CircuitBreaker(backend_id="ho_backend", ledger=ledger, failure_threshold=2, cooldown_sec=1)
