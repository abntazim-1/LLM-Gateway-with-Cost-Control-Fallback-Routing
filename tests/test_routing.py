...
async def test_router_cost_first_ranking():
    ...
    router = Router(
        adapters=[adapter_expensive, adapter_cheap],
...
async def test_router_excludes_tripped_circuit_breaker():
    ...
    router = Router(
        adapters=[adapter1, adapter2], circuit_registry=registry, strategy="cost_first"
    )
...
async def test_router_execute_stream():
    ...
    async for chunk in router.execute_stream(
        messages=[{"role": "user", "content": "hello"}], api_key="sk-stream-test"
    ):
        chunks.append(chunk)
