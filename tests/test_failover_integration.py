...
class SucceedsAdapter(BaseAdapter):
    ...
    async def complete(
        ...
        return NormalizedResponse(
            ...
            model=self.model,
            messages=[
                NormalizedMessage(role="assistant", content="Hello from backup!")
            ],
            prompt_tokens=5,
            completion_tokens=5,
            cost_usd=0.005,
...
async def test_router_failover_integration():
    ...
    registry = CircuitBreakerRegistry(
        ledger=ledger, failure_threshold=2, cooldown_sec=10
    )

    # Primary adapter will fail, secondary will succeed.
    primary = FailsAdapter(
        {
            "id": "primary",
            "model": "m1",
    ...
    router = Router(
        adapters=[primary, backup], circuit_registry=registry, strategy="cost_first"
    )
