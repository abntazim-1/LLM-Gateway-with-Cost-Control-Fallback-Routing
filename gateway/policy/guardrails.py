...
class GuardrailsPipeline:
    ...
    def __init__(self):
        ...
        self.output_leak_patterns = [
            re.compile(r"\bmy\s+system\s+prompt\s+(is|was|says|contains)\b", re.IGNORECASE),
            re.compile(r"\b(?:my|the)\s+(?:internal|hidden|original|secret)\s+instructions\s+(?:are|were)\b", re.IGNORECASE),
            re.compile(r"\bas\s+part\s+of\s+my\s+(?:system\s+)?instructions\b", re.IGNORECASE),
            re.compile(r"\bi\s+was\s+(?:instructed|told)\s+to\s+(?:never|always)\b", re.IGNORECASE),
        ]
